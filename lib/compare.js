...
function formatPersistentCacheHitRate(label, results) {
	...
	const cases = results.filter(
		({ name, result }) => name.includes("persistent-hot") && result[HIT_RATE_KEY] !== undefined
	);
...
export async function compare(base, current, benchmarkDirectory, repository = "web-infra-dev/rspack") {
	...
	const hitRateSummaries = [
		formatPersistentCacheHitRate("Base", baseResolved.results),
		formatPersistentCacheHitRate("Current", currentResolved.results),
	].filter(Boolean);
