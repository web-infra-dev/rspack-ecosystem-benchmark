...
import { PERSISTENT_CACHE_HIT_RATE_KEY } from "./addons/persistent-hot.js";
import {
	fetchBuildInfo,
	fetchIndex,
...
// Supports: "current" (default output dir), a local directory path, "YYYY-MM-DD" (date), or a branch name / commit hash.
...
function formatHitRateSummary(label, results) {
	const cases = results.filter(({ result }) => result[PERSISTENT_CACHE_HIT_RATE_KEY] !== undefined);
	...
	const misses = cases.filter(
		({ result }) =>
			result[PERSISTENT_CACHE_HIT_RATE_KEY].min !== 100 || result[PERSISTENT_CACHE_HIT_RATE_KEY].max !== 100
	);

	if (misses.length === 0) {
		return `${label} persistent cache hit rate: 👍`;
	}

	return [
		`${label} persistent cache hit rate:`,
		...misses
			.sort((a, b) => a.name.localeCompare(b.name))
			.map(({ name, result }) => `- ${name}: ${ratio(result[PERSISTENT_CACHE_HIT_RATE_KEY].mean)}`),
	].join("\n");
}
...
export async function compare(base, current, benchmarkDirectory, repository = "web-infra-dev/rspack") {
	...
	const hitRateSummaries = [
		formatHitRateSummary("Base", baseResolved.results),
		formatHitRateSummary("Current", currentResolved.results),
	].filter(Boolean);
