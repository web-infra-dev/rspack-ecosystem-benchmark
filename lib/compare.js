...
import { HIT_RATE } from "./addons/persistent-hot/index.js";
import {
	fetchBuildInfo,
	fetchIndex,
...
function formatHitRateSummary(label, results) {
	const cases = results.filter(({ name, result }) => result[HIT_RATE] !== undefined);

	if (cases.length === 0) {
		return `${label} persistent cache hit rate: N/A`;
	}

	const misses = cases.filter(({ result }) => result[HIT_RATE].min !== 1 || result[HIT_RATE].max !== 1);
	...
	return [
		...
		...misses
			...
			.map(({ name, hitRate }) => `- ${name}: ${(hitRate.mean * 100).toFixed(2)}%`),
	].join("\n");
}
