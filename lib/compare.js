...
function formatHitRateSummary(label, results) {
	const cases = results
		.map(({ name, result }) => {
			const metric = result[HIT_RATE];
			const hitRate =
				typeof metric === "number"
					? { min: metric, max: metric, mean: metric }
					: metric && typeof metric.mean === "number"
						? {
								min: metric.min,
								max: metric.max,
								mean: metric.mean
	...
	const misses = cases.filter(
		({ hitRate }) => hitRate.min !== 1 || hitRate.max !== 1
	);
...
export async function compare(base, current, benchmarkDirectory, repository = "web-infra-dev/rspack") {
	...
	const hitRateSummaries = [
		formatHitRateSummary(
			"Base",
			baseResolved.results
		),
		formatHitRateSummary(
			"Current",
			currentResolved.results
		)
	].filter(Boolean);
