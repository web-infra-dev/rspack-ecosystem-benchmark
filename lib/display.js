...
export function formatDiffTable(diff, { limit }) {
	...
	const columns = {
		name: l => l.name,
		base: () => 1,
		baseMean: l => f(l.name, l.baseMean),
		baseConfidence: l => f(l.name, l.baseConfidence),
		current: l => (l.currentMean / l.baseMean).toFixed(2),
		currentMean: l => f(l.name, l.currentMean),
		currentConfidence: l => f(l.name, l.currentConfidence)
	};
