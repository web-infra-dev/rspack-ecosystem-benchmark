...
export function formatDiffTable({
	...
	let baseTitle = `Base (${baseDate}`;
	if (baseCommitSHA) {
		baseTitle += ` [${baseCommitSHA.slice(0, 7)}](https://github.com/web-infra-dev/rspack/commit/${baseCommitSHA})`;
	}
	baseTitle += ")";

	const columns = {
		Name: l => l.name,
		[baseTitle]: l => `${f(l.name, l.baseMean)} ± ${f(l.name, l.baseConfidence)}`,
		Current: l =>
