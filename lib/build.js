...
export default (args) => {
	...
	const run = async (options) => {
		...
		for (const addon of options.addons) {
			...
			if (addon.nodeArgs) nodeArgs = nodeArgs.concat(addon.nodeArgs);
		}
		const p = spawn(
			nodeArgs.length === 0 ? "yarn" : "node",
			[
				...nodeArgs,
				nodeArgs.length === 0
					? "rspack"
				...
				...currentArgs,
				// TODO: add it back when rspack support this option
	...
	const setup = async (options) => {
		...
		await alterFile(
			...
			(content) => {
				...
				const finalWebpackConfig =  `${content}
...
`;
	...
	const teardown = async (options) => {
		await revertFile(rebuildChangeFile);
		await revertFile("rspack.config.js");
	};
	return {
		run,
		warmup,
