...
export default class BuildPlugin {
	apply(compiler) {
		...
		const isLazyCompilation = checkIsLazyCompilation(compiler.options);
		...
		compiler.hooks.done.tap("BuildPlugin", stats => {
			if (isWatching && !isLazyCompilation) {
				counter++;
			...
			if (isLazyCompilation && !firstRun) {
				// For lazy compilation, we need to exit the process after the last build
				process.nextTick(() => process.exit(0));
			}
