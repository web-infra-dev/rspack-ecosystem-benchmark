(async () => {
  ...
  debugger
	...
	const compareScenarioSelect = document.querySelector(
	...
	document.querySelector("#recent").addEventListener("change", (e) => {
		...
		axisTicks.min = e.target.checked
	...
	const updateChart = (inputDatasets) => {
		...
		for (const ds of inputDatasets) {
			const base =
				ds.entries.reduce(
					(sum, entry) => sum + (entry.data && entry.data.base),
					0,
				) / ds.entries.length;
			...
			let scale =
				sizeType || memoryType ? () => 1 : (entry) => base / entry.data.base;
			if (percentage) {
				const old = scale;
				const midFn = (entry) => (entry.data.low + entry.data.high) / 2;
				scale = (entry, fn) =>
					(old(entry, fn) / (midFn(lastEntry) * old(lastEntry, fn))) * 100;
			}
			const style = {
			...
			await Promise.all(
				dates.map(async (date) => {
					const data = await loadFile(
						`results/${date}/${testCase}_${scenario}.json`,
					);
	...
	const update = enqueued(async (ctx) => {
		...
		for (const testCase of caseSelect.value === "all"
			? testCases.keys()
			: [caseSelect.value]) {
			...
			if (caseSelect.value !== "all") {
				const compareData = await loadData(
				...
				if (
					(compareTestCase || compareScenario || compareMetric) &&
					compareData &&
					compareData.metrics.has(compareMetric || metric) &&
		...
		const updateSelect = (
			...
			for (const [metric, groupName, value] of sortedMetrics) {
				...
				option.innerText = `${metric} (${format(
					+value.toPrecision(4),
					value,
				)})`;
				option.value = metric;
